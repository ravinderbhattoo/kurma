...
from tnks import dynamic, static
...
class trajectory:
    ...
    def show_frame(self, index=None, frame=None):
        ...
        if index + 1 > self.nframes:
            print("index ({}) excedes total frames ({}) !!!".format(
                index, self.nframes))
        else:
    ...
    def make_chuncks(self, name='along_z', dir='z', step=None, start=None, end=None, n=2, logical='True'):
        ...
        if end < start:
            ...
            for ind, f in enumerate(self.frames):
                c = []
                for i in range(n):
                    c.append((f[dir] > start+i*step) &
                             (f[dir] < start+(i+1)*step) & eval(logical))
                chuncks.append(c)
...
def pdfs(df, id, self=None, ind=None, dim=3, *args, **kwargs):
    ...
    self.compute[id+'_'+list(pdf.PDF.keys())[0].split('_')
                 [0]] = list(pdf.PDF.values())
    if not((pdf.TCF == None) or (pdf.TCF == {})):
        self.compute[id+'_tcf_'+list(pdf.PDF.keys())
                     [0].split('_')[0]] = list(pdf.TCF.values())
    if not((pdf.SQ == None) or (pdf.SQ == {})):
        self.compute[id+'_sq_'+list(pdf.PDF.keys())
                     [0].split('_')[0]] = list(pdf.SQ.values())


def gr_n(df, id, self=None, ind=None, dim=3, *args, **kwargs):
    pdf = PDF(info=[self.frames, self.box, self.names], dim=dim)
    pdf.make_gr_n(frame_index=range(len(self.fn)), *args, **kwargs)
    self.compute[id+'_'+list(pdf.gr_n.keys())[0].split('_')
                 [0]] = list(pdf.gr_n.values())
...
def shi_n(df=None, pos=None, box=None, n=3, rm=2,):
    ...
    for ind, i in enumerate(pos):
        ...
        delta = delta[mask]
        delta = delta/l
        delta = np.where(delta > 1/2, delta-1, 0) + np.where(delta < -1/2,
                                                             1+delta, 0) + np.where((delta < -1/2)+(delta > 1/2), 0, delta)
